    ...
    <table class="table">
        <thead>
            <tr>
                <th>{{ __('messages.card_id') }}</th>
                        <th>{{ __('messages.security_code') }}</th>
        ...
        <tbody>
            @forelse($cards as $card)
                <tr>
                    <td>{{ $card->code }}</td>
                    <td>{{ $card->formattedSecurityCode() }}</td>
                    <td>
                    ...
                    <td>
                        <a href="{{ route('verify.show', ['locale' => $locale, 'verificationCard' => $card, 'securityCode' => $card->security_code]) }}">{{ __('messages.view_more') }}</a>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" style="text-align:center; color:#64748b; padding:2rem;">{{ __('messages.no_posts') }}</td>
                        </tr>
                    @endforelse
                </tbody>
                </table>
